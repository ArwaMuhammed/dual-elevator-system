...
/* Timeout value to prevent infinite hanging during a bus fault */
#define SPI_TIMEOUT_MAX 0x0FFFFU
...
uint8 Spi1_TransmitReceiveByte(uint8 TxData, uint8 *RxData)
{
  uint32 timeout = SPI_TIMEOUT_MAX;

  /* Wait for TX buffer to be empty with timeout */
  while (!(SPI1->SR & (1U << SPI_SR_TXE_Pos)))
  {
      if (--timeout == 0U) return SPI_NOK;
  }
