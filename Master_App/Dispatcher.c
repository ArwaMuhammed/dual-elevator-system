...
{
	/* Floor index is 0..3. Valid call buttons are: U1, D2, U2, D3, U3, D4 */
...
static DispatchClass_t Dispatcher_Classify(const ElevatorData_t *elevator,
										   uint8 callFloor,
										   ElevatorDir_t callDir,
										   uint8 *distance)
{
	uint8 dist = (callFloor > elevator->CurrentFloor)
				 ? (callFloor - elevator->CurrentFloor)
									   ...
									   const HallCall_t *call)
{
	...
	if (masterClass < slaveClass)
	{
		return DISPATCH_TARGET_MASTER;
	}
	if (slaveClass < masterClass)
	{
		return DISPATCH_TARGET_SLAVE;
	}

	if (masterDist < slaveDist)
	{
		return DISPATCH_TARGET_MASTER;
	}
	if (slaveDist < masterDist)
	{
		return DISPATCH_TARGET_SLAVE;
	}
										 ...
										 HallCall_t *outCall)
{
	...
	if (Dispatcher_PopNextHallCall(&call) == FALSE)
	{
		return DISPATCH_TARGET_NONE;
	}
	...
	{
		/* Put it back if no valid assignment yet */
		Dispatcher_AddHallCall(call.Floor, call.Direction);
		return DISPATCH_TARGET_NONE;
	}
